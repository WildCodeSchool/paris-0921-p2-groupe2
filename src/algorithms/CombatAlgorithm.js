  ...
  while (lifeB > 0 && lifeA > 0 && turns <= 15) {
    ...
    if (initiative == 'A') {
      ...
      if (damage <= -100) {
        initiativeA = -100;
        lifeA -= 3;
        rapport.push([`EPIC FAIL : ${fighterA.name} trips.`, Math.round((lifeA / initialLifeA) * 100), Math.round((lifeB / initialLifeB) * 100)]);
      } else if (damage < -50) {
        initiativeA /= 2;
        rapport.push([`${fighterB.name} dodges.`, Math.round((lifeA / initialLifeA) * 100), Math.round((lifeB / initialLifeB) * 100)]);
      } else if (damage < 0) {
        let punch = Math.floor(damageA * 0.1);
        lifeB -= punch;
        rapport.push([
          `${first ? fighterA.name : pronoun(fighterA)} is blocked.`,
          Math.round((lifeA / initialLifeA) * 100),
          Math.round((lifeB / initialLifeB) * 100),
        ]);
      } else if (damage < 50) {
        ...
        lifeB -= punch;
        rapport.push([`CRITICAL : ${fighterB.name} stumbles.`, Math.round((lifeA / initialLifeA) * 100), Math.round((lifeB / initialLifeB) * 100)]);
      }
      ...
      if (damage <= -100) {
        initiativeB = -100;
        lifeB -= 3;
        rapport.push([`EPIC FAIL : ${fighterB.name} trips.`, Math.round((lifeA / initialLifeA) * 100), Math.round((lifeB / initialLifeB) * 100)]);
      } else if (damage < -33) {
        initiativeB /= 2;
        rapport.push([`${fighterA.name} dodges.`, Math.round((lifeA / initialLifeA) * 100), Math.round((lifeB / initialLifeB) * 100)]);
      } else if (damage < 0) {
        ...
        lifeA -= punch;
        rapport.push([`CRITICAL : ${fighterA.name} stumbles.`, Math.round((lifeA / initialLifeA) * 100), Math.round((lifeB / initialLifeB) * 100)]);
      }
    }
  }
  if (lifeA <= 0) {
